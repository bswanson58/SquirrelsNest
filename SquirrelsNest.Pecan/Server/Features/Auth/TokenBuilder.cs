...
namespace SquirrelsNest.Pecan.Server.Features.Auth {
    ...
    public class TokenBuilder : ITokenBuilder {
        ...
        public TokenBuilder( UserManager<DbUser> userManager, IConfiguration configuration ) {
            ...
            mJwtSettings = configuration.GetSection( "JWTSettings" );
        }

        private SigningCredentials GetSigningCredentials() {
            var key = Encoding.UTF8.GetBytes( mJwtSettings["securityKey"] ?? String.Empty );
        ...
        private JwtSecurityToken GenerateTokenOptions( SigningCredentials signingCredentials, List<Claim> claims ) {
            var tokenOptions = new JwtSecurityToken(
                issuer: mJwtSettings["validIssuer"],
                audience: mJwtSettings["validAudience"],
                claims: claims,
        ...
        public DateTime TokenExpiration() =>
             DateTimeProvider.Instance.CurrentDateTime.AddMinutes( Convert.ToDouble( mJwtSettings["expiryInMinutes"]));
        ...
        public ClaimsPrincipal GetPrincipalFromExpiredToken( string token ) {
            ...
            var tokenValidationParameters = new TokenValidationParameters {
                ...
                IssuerSigningKey = new SymmetricSecurityKey( Encoding.UTF8.GetBytes( mJwtSettings["securityKey"] ?? String.Empty )),
                ValidateLifetime = false,
                ValidIssuer = mJwtSettings["validIssuer"],
                ValidAudience = mJwtSettings["validAudience"],
            };
